...
var (
	logger                = logrus.WithField("storage-driver", "manuka")
	backingFs             = "<unknown>"
...
func Init(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (graphdriver.Driver, error) {
	// "home" is in the format of /var/lib/docker/<plugin-name>
	...
	// Perform feature detection on /var/lib/docker/<plugin-name>. Create the directory if it does not exist
	testdir := home
	if _, err := os.Stat(testdir); os.IsNotExist(err) {
		if err := idtools.MkdirAllAndChown(testdir, 0700, idtools.Identity{UID: rootUID, GID: rootGID}); err != nil {
			return nil, err
		}
	}
	...
	case graphdriver.FsMagicAufs, graphdriver.FsMagicEcryptfs, graphdriver.FsMagicNfsFs, graphdriver.FsMagicOverlay, graphdriver.FsMagicZfs:
		logger.Errorf("'%s' is not supported over %s", driverName, backingFs)
	...
	case graphdriver.FsMagicBtrfs:
		...
		if kernel.CompareKernelVersion(*v, kernel.VersionInfo{Kernel: 4, Major: 7, Minor: 0}) < 0 {
			if !opts.overrideKernelCheck {
				logger.Errorf("'%s' requires kernel 4.7 to use on %s", driverName, backingFs)
				return nil, graphdriver.ErrIncompatibleFS
			}
	...
	// Create the driver home dir and the link dir /var/lib/docker/<plugin-name>/<linkDir>
	...
	logger.Infof("[Init] Driver '%s' initialized", driverName)
...
func (d *Driver) Status() [][2]string {
	logger.Infof("[Status] Get graphdriver status")
	return [][2]string{
...
func (d *Driver) GetMetadata(id string) (map[string]string, error) {
	...
	logger.Infof("[GetMetadata] Get metadata of layer %s", id)
...
func (d *Driver) CreateReadWrite(id, parent string, opts *graphdriver.CreateOpts) error {
	...
	logger.Infof("[CreateReadWrite] Create read-write layer %s", id)
...
func (d *Driver) Create(id, parent string, opts *graphdriver.CreateOpts) (retErr error) {
	...
	logger.Infof("[Create] Create read-only layer %s", id)
	return d.create(id, parent, opts)
}
...
func (d *Driver) Remove(id string) error {
	...
	logger.Infof("[Remove] Remove layer %s", id)
	return nil
}
...
func (d *Driver) Get(id, mountLabel string) (_ containerfs.ContainerFS, retErr error) {
	...
	logger.Infof("[Get] Read lower file")
	lowers, err := ioutil.ReadFile(path.Join(dir, lowerFile))
	if err != nil {
		// If no lower, just return diff directory
		if os.IsNotExist(err) {
			logger.Debugf("[Get] Lower file does not exist")
			return containerfs.NewLocalContainerFS(diffDir), nil
		}
		logger.Infof("[Get] Other error: %s", err)
		return nil, err
	}

	mergedDir := path.Join(dir, "merged")
	if count := d.ctr.Increment(mergedDir); count > 1 {
		logger.Infof("[Get] MergedDir already exists, return")
	...
	rootUID, rootGID, err := idtools.GetRootUIDGID(d.uidMaps, d.gidMaps)
	if err != nil {
		logger.Infof("[Get] Failed to get root UID/GID")
		return nil, err
	}
	logger.Infof("[Get] MergedDir: %s", mergedDir)
	if err := idtools.MkdirAllAndChown(mergedDir, 0700, idtools.Identity{UID: rootUID, GID: rootGID}); err != nil {
		logger.Infof("[Get] Failed to create mergedDir %s", mergedDir)
		return nil, err
	}
	...
	logger.Infof("[Get] Data size: %d, page size: %d", len(mountData), pageSize)
	...
	logger.Infof("[Get] Create and mount the layered filesystem for %s", id)
	return containerfs.NewLocalContainerFS(mergedDir), nil
}
...
func (d *Driver) Put(id string) error {
	...
	logger.Infof("[Put] Release resources for layer %s", id)
	return nil
}
...
func (d *Driver) Exists(id string) bool {
	...
	logger.Infof("[Exists] Determine if layer %s exists", id)
	return err == nil
}
...
func (d *Driver) ApplyDiff(id string, parent string, diff io.Reader) (size int64, err error) {
	...
	size, err = directory.Size(context.TODO(), applyDir)
	logger.Infof("[ApplyDiff] Apply diff for layer %s, size %d", id, size)
	return size, err
}
...
func (d *Driver) DiffSize(id, parent string) (size int64, err error) {
	...
	logger.Infof("[DiffSize] Layer %s diff size %d", id, size)
	return size, err
}
...
func (d *Driver) Diff(id, parent string) (io.ReadCloser, error) {
	...
	logger.Infof("[Diff] Get diff of layer %s", id)
...
func (d *Driver) Changes(id, parent string) ([]archive.Change, error) {
	logger.Infof("[Changes] Find changes for layer %s", id)
